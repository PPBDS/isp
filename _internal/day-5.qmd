...
Goal is to download the social capital data by hand and start to wrangle it.  Need to highlight that the material assigned in the tutorials is used in class, so doing the tutorials is a good idea. Let them struggle with the key step of creating a new state variable. Expose them to ChatGPT.

My repo from the last class, which was created on the class organization account, is used to start everyone's work, via fork. I show them the fork as if I were a student.
...
Before loading up with this new data, let's take a look at the connectedness tibble from last week.  Explore the data.
...
Send to the rooms and interactively find the information for your county, or for a weird county.
